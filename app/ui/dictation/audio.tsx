...
export default function Audio({ dictation, audioIndexParam }: AudioProps) {
  ...
  return (
    ...
    <div className="max-w-[30rem] bg-gray-300 p-2 mt-8 rounded shadow-md z-10 flex-1">
      <audio ref={audioRef} src={audioSrc} className="w-full"></audio>
      <div id="main" className="flex items-center justify-between">


        <button id="player" className="flex items-center justify-center w-8 h-8 rounded-full bg-green-500 text-white focus:outline-none"
          onClick={togglePlay}
        >
          {isPlaying ? (
        ...
        <div id="progressEtchange" className="flex-1 flex flex-grow">

          <button id="retour" className="mr-4 focus:outline-none"
          ...
          <div id="middle" className="flex-1 flex flex-col">
            <div id="middleText" className="flex-1 flex items-center justify-between text-sm text-gray-600">
              <div className="flex-1"></div>
              <div className="flex-[2] flex justify-center">
                <p>{state.audioIndex}/{dictation.audio_total_part}</p>
              </div>
              <div className="flex-1 flex justify-end">
                {!isNaN(duration.minutes) && !isNaN(duration.seconds) ? (
                  <p className="text-xs">{duration.minutes} m {duration.seconds} s</p>
                ) : null}
              </div>

            </div>

            <div id="progressbar" className="flex-1 w-full  bg-gray-300 rounded cursor-pointer"
              onClick={handleProgressClick}
              onMouseUp={handleProgressChangeEnd}
              onMouseLeave={handleProgressChangeEnd}
            >
              <div
                className={`h-2 ${isProgressChanging ? 'bg-blue-500' : 'bg-green-500'
                  } rounded`}
          ...
          <button id="next" className="ml-4 focus:outline-none"
            onClick={() => handleAudioChange(state.audioIndex + 1)}
        ...
        <div id="param" className="flex items-center">
